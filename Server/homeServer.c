...
#include "Keypad.h"         // ReadInputFromKeypad(), ParseKeypadInput()
...
char *SENSOR_NAME[NUMBER_OF_SENSOR]    = {"Egim", "Mesafe"};    // Sensor Tipleri
int   SENSOR_PINS[NUMBER_OF_SENSOR]    = {SENSOR_1, SENSOR_2};  // Sensor Pinleri
...
pthread_t threadID_ListenKeypad;
...
/*** THREAD'LER TARAFINDAN ISLETILEN FONKSIYONLAR ***/

/* *
 * Keypad dinlenerek, gerekli islemler yapılır.
 * */
void *threadListenKeypad(void *arg) {
    ...
    while (1)   {
        ...
        ReadInputFromKeypad(11, keypadInput);
...
int main(int argc, char **argv) {
    ...
    //  Sürekli keypadi dinleyecek thread yaratiliyor
    pthread_create(&threadID_ListenKeypad, NULL, threadListenKeypad, NULL);
    ...
    // Ana proses'in bitmesini engellemek icin herhangi bir thread'i bekliyoruz.
    pthread_join(threadID_ListenKeypad, NULL);
...
/**********************************************
 * KEYPAD ISLEMLERİ İCİN GEREKLİ FONKSİYONLAR *
 **********************************************/
/* *
 * Keypad'ten alinan girdiye gore uygun islemleri gerceklestirir.
 * */
void DoKeypadOperations(char* keypadInput){
    ...
    isParse = ParseKeypadInput(keypadInput, currentPasswd, newPasswd, chStar, chSharp, relayStatus);

    // Parse Edildi ise
    if (isParse){

        // Mevcut Parola ve "*" karateri mevcut ise
...
/* *
 * Mevcut parolanin dogrulugunu kontrol eder
 * */
...
void SignalHandlerShutdown(int signalNo)
{
    if( signalNo == SIGINT ){
        ...
        pthread_cancel(threadID_ListenKeypad);
